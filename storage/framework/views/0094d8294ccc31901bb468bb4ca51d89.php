...
<div class="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-4 mb-6">
    <form action="" method="GET" class="flex gap-4">
        <div class="flex-1">
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="البحث بالاسم أو البريد..." 
        ...
        <?php if(request('search')): ?>
            <a href="<?php echo e(route('admin.users.index')); ?>" class="bg-slate-700 text-white px-6 py-2.5 rounded-xl font-medium transition-all duration-200 hover:bg-slate-600">إلغاء</a>
        <?php endif; ?>
    </form>
</div>
                    ...
                    <tr class="hover:bg-slate-800/30 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <?php if($user->avatar): ?>
                                    <img src="<?php echo e($user->avatar); ?>" alt="" class="w-10 h-10 rounded-full object-cover">
                                <?php else: ?>
                        ...
                        <td class="px-6 py-4">
                            <?php if($user->country): ?>
                                <span class="text-slate-300"><?php echo e($user->country->flag); ?> <?php echo e($user->country->name_ar); ?></span>
                            <?php else: ?>
                                <span class="text-slate-500">-</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-slate-300"><?php echo e($user->subscriptions_count); ?></td>
                        <td class="px-6 py-4">
                            <?php if($user->last_login_at): ?>
                                <span class="text-slate-400 text-sm"><?php echo e($user->last_login_at->diffForHumans()); ?></span>
                            <?php else: ?>
                                <span class="text-slate-500">-</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-slate-400 text-sm"><?php echo e($user->created_at->format('Y-m-d')); ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <a href="<?php echo e(route('admin.users.show', $user)); ?>" class="text-slate-400 hover:text-sky-400 transition-colors">
...
<?php echo $__env->make('admin.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Newsly\backend\resources\views/admin/users/index.blade.php ENDPATH**/ ?>