...
<div class="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-4 mb-6">
    <form action="" method="GET" class="grid grid-cols-1 md:grid-cols-6 gap-4">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="البحث في العنوان..." 
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.articles.index') }}" class="bg-slate-700 text-white px-6 py-2.5 rounded-xl font-medium transition-all duration-200 hover:bg-slate-600">إلغاء</a>
        </div>
    </form>
</div>
        ...
        <div class="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl overflow-hidden group">
            ...
            <div class="p-4">
                ...
                <div class="flex items-center justify-between">
                    <span class="text-xs text-slate-500">{{ number_format($article->views_count) }} مشاهدة</span>
                    
                    <div class="flex items-center gap-2">
                        <form action="{{ route('admin.articles.toggle-breaking', $article) }}" method="POST" class="inline">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="text-slate-400 hover:text-amber-400 transition-colors" title="{{ $article->is_breaking ? 'إلغاء عاجل' : 'تعيين عاجل' }}">
