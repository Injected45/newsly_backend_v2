...
<div class="max-w-3xl">
    <div class="card">
        <div class="p-6 border-b border-dark-700/50">
            <h2 class="text-lg font-bold">إضافة مصدر جديد</h2>
        </div>
        
        <form action="{{ route('admin.sources.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="url" name="rss_url" value="{{ old('rss_url') }}" class="input" placeholder="https://example.com/rss" required>
                @error('rss_url')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="slug" value="{{ old('slug') }}" class="input" placeholder="source-name" required>
                    @error('slug')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="website_url" value="{{ old('website_url') }}" class="input" placeholder="https://example.com">
                    @error('website_url')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="url" name="logo" value="{{ old('logo') }}" class="input" placeholder="https://example.com/logo.png">
                    @error('logo')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="fetch_interval_seconds" value="{{ old('fetch_interval_seconds', 300) }}" class="input" min="60" max="86400">
            ...
            <div class="flex flex-col gap-4">
                ...
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_breaking_source" value="1" {{ old('is_breaking_source') ? 'checked' : '' }} class="w-5 h-5 rounded border-dark-600 bg-dark-900 text-primary-500 focus:ring-primary-500/20">
